...
public class Agent {
	...
	private Path selectPath() {
		...
		// Evaluate Cells
		/////////////////
		Vector<Point> cells = mapBuilder.getMap().getCellsWithinDistance(
				status.coordinates, 1000);
		PriorityQueue<PointValue> weights = evaluateCells(cells);
		...
		////////////////////////
		Path result = mapBuilder.getPlanner().pathPlan(status, new Point(row, col));
		mapBuilder.getPlanner().clearCache();
		...
		if(env.options.debugMode) {
			...
			for (Integer i : maxs) {
				env.maxs.add(collections.points.get(i));
			}
		}
	...
	private PriorityQueue<PointValue> evaluateCells(Vector<Point> cells) {
